...
def get_image(id: int,
    ...
    try:
        image = image_service(image_repo=image_repo,
                              session=session()
...
def create_image(image_service=ImageService,
                 image_repo=ImageRepository,
                 image_schema=ImageSchemaAdd,
                 session=Session
                 ):
...
def delete_image(id: int,
    ...
    try:
        response = image_service(image_repo=image_repo,
                                 session=session()
                                 ).delete_image({'id': id})
    except:
        return make_response(jsonify({'error': 'Error delete image'}),
                             404
                             )
