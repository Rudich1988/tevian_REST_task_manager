...
def get_task(id: int,
    ...
    try:
        task = task_service(task_repo=task_repo,
                            session=session()
                            ).get_task(task_data={'id': id},
                                       schema=task_schema
                                       )
    except:
        return make_response(jsonify({'error': 'Error get task'}),
                             404
                             )
    return jsonify(task)
...
def delete_task(id: int,
    ...
    try:
        response = task_service(task_repo=task_repo,
                                session=session()
                                ).delete_task({'id': id})
    except:
        return make_response(jsonify({'error': 'Error delete task'}),
                             404
                             )
